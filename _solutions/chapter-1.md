...
<a name="ex1.2"></a>
## Exercise 1.2.
There is no set $X$ such that $P(X) \subseteq X$.
...
<a name="ex1.3"></a>
## Exercise 1.3.
If $X$ is inductive, then the set $\\{x \in X : x \subseteq X\\}$ is inductive. Hence $\boldsymbol{N}$ is transitive, and for each $n$, $n = \\{m \in \boldsymbol{N} : m < n\\}$.
...
<a name="ex1.4"></a>
## Exercise 1.4.
If $X$ is inductive, then the set $\\{x \in X : x \text{ is transitive}\\}$ is inductive. Hence every $n \in \boldsymbol{N}$ is transitive.
...
<a name="ex1.5"></a>
## Exercise 1.5.
If $X$ is inductive, then the set $\\{x \in X : x \text{ is transitive and } x \notin x\\}$ is inductive. Hence $n \notin n$ and $n \neq n + 1$ for each $n \in \boldsymbol{N}$.

<i>Solution.</i> Clearly $\emptyset$ is transitive and $\emptyset \notin \emptyset$ as $\emptyset$ has no elements. Now suppose $x \in X$. In the proof of [Exercise 1.4](https://clementyung.github.io/jech-solutions/chapter-1#ex1.4) we have already seen that $x \cup \\{x\\}$ is transitive. It remains to show that $x \cup \\{x\\} \notin x \cup \\{x\\}$.
...
<a name="ex1.6"></a>
## Exercise 1.6.
If $X$ is inductive, then $\\{x \in X : x$ is transitive and every nonempty $z \subseteq x$ has an $\in$-minimal element$\\}$ is inductive ($t$ is <b>$\in$-minimal</b> in $z$ if there is no $s \in z$ such that $s \in t$).

<i>Solution.</i> Clearly $\emptyset$ is transitive and since it has no non-empty subset, the second requirement holds trivially. Now suppose $x$ is transitive and every non-empty $z \subseteq x$ has an $\in$-minimal element. In the proof of [Exercise 1.4](https://clementyung.github.io/jech-solutions/chapter-1#ex1.4) we have already seen that $x \cup \\{x\\}$ is transitive.
...
<a name="ex1.7"></a>
## Exercise 1.7.
Every nonempty $X \subseteq \boldsymbol{N}$ has an $\in$-minimal element.

\begin{hint}
Pick $n \in X$ and look at $X \cap n$.
\end{hint}

<i>Solution.</i> Let $n \in X$. Then $X \cap n \subseteq n$, and by [Exercise 1.6](https://clementyung.github.io/jech-solutions/chapter-1#ex1.6) we have that $X \cap n$ has a $\in$-minimal element. Let $m \in X \cap n$ be such a $\in$-minimal element. We shall show that $m$ is the $\in$-minimal element in $X$.

Suppose not, so there exists a $m' \in X$ such that $m' \in m$. By [Exercise 1.3](https://clementyung.github.io/jech-solutions/chapter-1#ex1.3), we have that $m = \\{p \in \boldsymbol{N} : p \in m\\}$. Since $m \in n$, by the same Exercise we have $p \in n$. Thus, $m' \in X \cap n$, so $m$ is not $\in$-minimal in $X \cap n$, a contradiction. 
$\square$

<a name="ex1.8"></a>
## Exercise 1.8.
If $X$ is inductive then so is $\\{x \in X : x = \emptyset$ or $x = y \cup \\{y\\}$ for some $y\\}$. Hence each $n \neq 0$ is $m + 1$ for some $m$.
...
<a name="ex1.9"></a>
## Exercise 1.9.
<b>(Induction).</b> Let $A$ be a subset of $\boldsymbol{N}$ such that $0 \in A$, and if $n \in A$ then $n + 1 \in A$. Then $A = \boldsymbol{N}$.

<i>Solution.</i> This follows from that $\boldsymbol{N}$ is inductive and [Exercise 1.8](https://clementyung.github.io/jech-solutions/chapter-1#ex1.8). 
$\square$

<a name="ex1.10"></a>
## Exercise 1.10.
\begin{definition}
A set $X$ <b>has $n$ elements</b> (where $n \in \boldsymbol{N}$) if there is a one-to-one mapping of $n$ onto $X$. A set is <b>finite</b> if it has $n$ elements for some $n \in \boldsymbol{N}$, and <b>infinite</b> if it is not finite.
\end{definition}

\begin{definition}
A set $S$ is <b>T-finite</b> if every nonempty $X \subseteq P(S)$ has a $\subseteq$-maximal element, i.e., $u \in X$ such that there is no $v \in X$ with $u \subseteq v$ and $u \neq v$. $S$ is T-<b>infinite</b> if it is not $T$-finite. (T is for Tarski.)
\end{definition}

Each $n \in \boldsymbol{N}$ is T-finite.

<i>Solution.</i> Let $A := \\{n \in \boldsymbol{N} : n \text{ is T-finite}\\}$. Clearly $0 = \emptyset \in A$ as it is vacuously T-finite. Suppose $n$ is T-finite. We shall show that $n + 1$ has $n$ as its $\subseteq$-maximal element, and is hence T-finite. Let $m \in n + 1 = n \cup \\{n\\}$. If $m \in \\{n\\}$, then $m = n$ so $m \subseteq n \cup \\{n\\}$ immediately. If $m \in n$, then let $m' \in n$ be its $\subseteq$-maximal element. Since $n$ is transitive by [Exercise 1.4](https://clementyung.github.io/jech-solutions/chapter-1#ex1.4), we have $m \subseteq m' \subseteq n \subseteq n + 1$. Thus $n + 1$ is T-finite. By induction ([Exercise 1.9](https://clementyung.github.io/jech-solutions/chapter-1#ex1.9)), $A = \boldsymbol{N}$ so every $n \in \boldsymbol{N}$ is T-finite. 
$\square$

<a name="ex1.11"></a>
## Exercise 1.11.
$\boldsymbol{N}$ is T-infinite - the set $\boldsymbol{N} \subseteq P(\boldsymbol{N})$ has no $\subseteq$-maximal element.

<i>Solution.</i> Suppose $\boldsymbol{N}$ is T-finite. Let $n \in \boldsymbol{N}$ be $\subseteq$-maximal. Then $n \cup \\{n\\} \in \boldsymbol{N}$ and therefore $n \cup \\{n\\} \subseteq n$. Hence $n \in n$, contradicting [Exercise 1.4](https://clementyung.github.io/jech-solutions/chapter-1#ex1.4). 
$\square$

<a name="ex1.12"></a>
## Exercise 1.12.
Ever finite set is $T$-finite.
...
\begin{hint}
If $S$ is infinite, consider $X = \\{u \subseteq S : u \text{ is finite}\\}$.
\end{hint}
...
<i>Solution.</i> Let $S$ be infinite and let $X$ be the set in the hint. Let $u \in X$. Since $u$ is finite and $X$ is infinite, $u \neq X$. Thus, there exists some $x \in X \setminus u$. By [Lemma 1.13.A](https://clementyung.github.io/jech-solutions/chapter-1#lem1.13.A), $u \cup \\{x\\}$ has $n + 1$ and is in particular finite, so $u \cup \\{x\\}$. Then $u \subsetneq u \cup \\{x\\}$. 
$\square$
...
\begin{hint}
Given $\varphi$, let $F = \\{(x,x) : \varphi(x)\\}$. Then $\\{x \in X : \varphi(x)\\} = F(X)$, for every $X$.
\end{hint}
...
\begin{tabular}{ll@{\hspace{0.5\tabcolsep}}l}
(1.8) & $\forall X \, \exists Y \, \bigcup X \subseteq Y$, & i.e. $\forall X \, \exists Y \, (\forall x \in X)(\forall u \in x) \, u \in Y$. \\
(1.9) & $\forall X \, \exists Y \, P(X) \subseteq Y$, & i.e. $\forall X \, \exists Y \, \forall u \, (u \subseteq X \to u \in Y)$. \\
(1.10) & If a class $F$ is a function, & then $\forall X \, \exists Y \, F(X) \subseteq Y$.
\end{tabular}

Then axioms 1.4, 1.5, 1.7 can be proved from (1.8), (1.9) and (1.10), using the Separation Schema (1.3).
