...
<a name="ex9.4"></a><br/>
<h2>Exercise 9.4.</h2>
<em>Solution.</em> Suppose for a contradiction that $s \in [\kappa]^\omega$ is a homogeneous set for the function $F$ defined in the hint. It's easy to check that $\equiv$ is an equivalence relation. Let $t$ be the representative of the equivalence class containing $s$. Let $x \in s$ be an arbitrary element, and let $s' := x - \lbrace x \rbrace$. Then:
$$
\begin{align*}<br/>
\lbrace n : s(n) \neq t(n) \rbtrace \text{ is even} \iff \lbrace n : s'(n) \neq t(n) \rbtrace \text{ is odd}
\end{align*}$$
In other words, $F(s) = 0$ iff $F(s) = 1$. This contradicts the homogeneity of $s$.<p align="right">$\square$</p><br/>
<br/>
<a name="ex9.5"></a><br/>
<h2>Exercise 9.5.</h2>
