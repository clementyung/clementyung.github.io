...
<div class="semester-section">
  <a name="ex11.1"></a>
  <h3 class="semester-title">Exercise 11.1</h3>
  <div class="solution-box">
    <span class="status-badge success">Solution</span>
    <br/>
    Let $\lbrace A_n : n < \omega\rbrace$ be a countable collection of sets.<br/>
    <br/>
    <u>Countable union:</u> For $s \in \Seq$, let:
    $$
    \begin{align*}
    A_s =
    \begin{cases}
    ...
    \end{align*}$$
    <u>Countable intersection:</u> For $s \in \Seq$, let:
    $$
    \begin{align*}
    A_s := A_{\length(s)}
    \end{align*}$$
...
<div class="semester-section">
  <a name="ex11.2"></a>
  <h3 class="semester-title">Exercise 11.2</h3>
  <div class="solution-box">
    <span class="status-badge success">Solution</span>
    <br/>
    We prove the identity in the hint. Since $\lbrace A_s : \func{length}(s) = n\rbrace$ is countable, the result follows.<br/>
    <br/>
    ...
    <div class="lemma-box" style="margin-top: 12px;">
      ...
      <br/>
      For each $n$, $s_n$ is an initial segment of $s_{n+1}$.
      <hr/>
      <span class="status-badge lemma">Proof</span>
      <br/>
      Suppose this fails at some $n \geq 1$. Let:
      $$
      \begin{align*}
...
<div class="semester-section">
  <a name="ex11.4"></a>
  <h3 class="semester-title">Exercise 11.4</h3>
  <div class="solution-box">
    <span class="status-badge success">Solution</span>
    <br/>
    There is probably a typo in the problem, as we can simply let each $G_n$ be an open interval around $a_n$ of small enough size and the assertion follows. Of course $G_n$ can have rational endpoints. Thus, we do the problem with the requirement $A \subseteq \bigcap_{n=0}^\infty G_n$ instead.<br/>
    <br/>
    We first define $G_0$. Let $J$ be an open interval (with rational endpoints) around $0$ of measure $< \frac{a_0}{2}$. Since $\lim_n a_n = 0$, all but finitely many of the points in the set $\lbrace a_i : i < \omega\rbrace$ lie in $J$. Enumerate the uncovered points $\lbrace a_{i_0},\dots,a_{i_k}\rbrace$. Let $I_i$ be an open interval (with rational endpoints) around $a_i$ of measure $< \frac{a_0}{2(k + 1)}$. Let:
    $$
    \begin{align*}
...
<div class="semester-section">
  <a name="ex11.5"></a>
  <h3 class="semester-title">Exercise 11.5</h3>
  <div class="lemma-box">
    <a name="lem11.5.A"></a>
    <span class="status-badge lemma">Lemma 11.5.A</span>
    <br/>
    Every meager set is included in a meager $F_\sigma$ set.
    <hr/>
    <span class="status-badge lemma">Proof</span>
    <br/>
    Let $M$ be a meager set, so $M = \bigcup_{i=0}^\infty X_i$, where $X_i$ is nowhere dense for all $i$. Let $M' := \bigcup_{i=0}^\infty \cl(X_i)$. Since the closure of a nowhere dense set is nowhere dense, $M'$ is meager. Clearly $M'$ is $F_\sigma$.
    <p align="right">$\blacksquare$</p>
  </div>
  <div class="lemma-box">
    <a name="lem11.5.B"></a>
    <span class="status-badge lemma">Lemma 11.5.B</span>
    <br/>
    If $U$ is open, then $\del U := \bar{U} - U$ is nowhere dense.
    <hr/>
    <span class="status-badge lemma">Proof</span>
    <br/>
    Note that $\del U$ is closed. Observe furthermore that $\del U = \bar{U} \cap U^c$. Thus:
    $$
    \begin{align*}
  ...
  <div class="solution-box">
    <span class="status-badge success">Solution</span>
    <br/>
    Let $A$ be a set with the Baire property, so $A = U \symdiff M$ for some open $U$ and meager $M$. By <a href="#lem11.5.A">Lemma 11.5.A</a>, we have that $M \subseteq M'$ for some meager $F_\sigma$ set $M'$. Let $F := \bar{U} \cup M'$. Clearly $F$ is $F_\sigma$. We shall show that $F - A$ is meager.<br/>
    <br/>
    Note that every subset of a meager set is meager. Observe that:
    $$
    \begin{align*}
    ...
    <br/>
    On the other hand, let $G := U - M' \subseteq U - M \subseteq A$. Since $M'$ is $F_\sigma$, $U - M'$ is $G_\delta$. Then $A - G$ is meager, as:
    $$
    \begin{align*}
    A - G &= ((U - M) \cup (M - U)) - (U - M') \\
...
<div class="semester-section">
  <a name="ex11.6"></a>
  <h3 class="semester-title">Exercise 11.6</h3>
  <div class="solution-box">
    <span class="status-badge success">Solution</span>
    <br/>
    Write $A = U \symdiff M$ where $U$ is open and $M$ is meager. Let $U' := \int(\bar{U})$. Then:
    $$
    \begin{align*}
    ...
    <br/>
    We now show that such a $U'$ is unique. Suppose $U_1$ and $U_2$ are regular open sets such that both $A \symdiff U_1$ and $A \symdiff U_2$ are meager. Note that this is equivalent to saying that $A = U_1 \symdiff M_1 = U_2 \symdiff M_2$ for meager sets $M_1$ and $M_2$, which gives $U_1 \symdiff U_2 = M_1 \symdiff M_2$. Thus:
    $$
    \begin{align*}
    U_1 - \bar{U_2} \subseteq U_1 - U_2 \subseteq U_1 \symdiff U_2 \subseteq M_1 \symdiff M_2
    \end{align*}$$
    Now $M_1 \symdiff M_2$ is meager, so $U_1 - \bar{U_2}$ is a meager open set, thus empty (by the Baire category theorem). This implies that $U_1 \subseteq \int(\bar{U_2}) = U_2$. This argument is symmetric between $U_1$ and $U_2$, so equality holds.
    <p align="right">$\square$</p>
  </div>
</div>
...
<div class="semester-section">
  <a name="ex11.8"></a>
  <h3 class="semester-title">Exercise 11.8</h3>
  <div class="solution-box">
    <span class="status-badge success">Solution</span>
    <br/>
